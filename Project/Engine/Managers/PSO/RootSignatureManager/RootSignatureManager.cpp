...
///					ルートシグネチャの作成
/// -------------------------------------------------------------
Microsoft::WRL::ComPtr<ID3D12RootSignature> RootSignatureManager::CreateRootSignature(DirectXCommon* dxCommon, PipelineType pipelineType)
{
	HRESULT hr = S_FALSE;


	/// -------------------------------------------------------------
	///			3Dオブジェクトのルートシグネチャの作成
	/// -------------------------------------------------------------
	if (pipelineType == NormalObject3D)
	{
		...
		// バイナリをもとにルートシグネチャ生成
		rootSignature[pipelineType] = nullptr;
		hr = dxCommon->GetDevice()->CreateRootSignature(0, signatureBlob->GetBufferPointer(), signatureBlob->GetBufferSize(), IID_PPV_ARGS(&rootSignature[pipelineType]));
		assert(SUCCEEDED(hr));
	}
	...
	return rootSignature[pipelineType];
}
