...
inline InputSnapshot BuildInputSnapshot(Ken4lowEngine::Input& input)
{
	...
	// ---- Fire Mode Toggle (V / Pad Y) ----
	s.toggleFireModePressed = input.TriggerKey(DIK_V);
	if (input.IsConnect())
	{
		s.toggleFireModePressed = s.toggleFireModePressed || input.TriggerButton(XButtons.Y);
	}
	...
	// ---- Weapon Category Slot (1..6) ----
	// 1=Primary,2=Backup,3=Melee,4=Special,5=Sniper,6=Heavy
	if (input.TriggerKey(DIK_1) || input.TriggerKey(DIK_NUMPAD1)) s.weaponSlotPressed = 1;
	else if (input.TriggerKey(DIK_2) || input.TriggerKey(DIK_NUMPAD2)) s.weaponSlotPressed = 2;
	else if (input.TriggerKey(DIK_3) || input.TriggerKey(DIK_NUMPAD3)) s.weaponSlotPressed = 3;
	else if (input.TriggerKey(DIK_4) || input.TriggerKey(DIK_NUMPAD4)) s.weaponSlotPressed = 4;
	else if (input.TriggerKey(DIK_5) || input.TriggerKey(DIK_NUMPAD5)) s.weaponSlotPressed = 5;
	else if (input.TriggerKey(DIK_6) || input.TriggerKey(DIK_NUMPAD6)) s.weaponSlotPressed = 6;
