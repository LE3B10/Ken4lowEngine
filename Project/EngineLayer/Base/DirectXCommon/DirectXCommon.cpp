...
#pragma endregion
	...
	void DirectXCommon::CreateShadowMapResources(bool allocateDescriptor)
	{
		...
		shadowMapResource_ = DSVManager::GetInstance()->CreateShadowMapResource(shadowMapWidth_, shadowMapHeight_);
		...
		shadowMapViewport = D3D12_VIEWPORT(0.0f, 0.0f, (float)shadowMapWidth_, (float)shadowMapHeight_, 0.0f, 1.0f);
		shadowMapScissorRect = D3D12_RECT(0, 0, static_cast<LONG>(shadowMapWidth_), static_cast<LONG>(shadowMapHeight_));
