...
{
	// 背景
	resultBackground_ = std::make_unique<K4E::Sprite>();
	...
	// リスタートテキスト
	restartText_ = std::make_unique<K4E::Sprite>();
	...
	// タイトルへ戻るテキスト
	titleText_ = std::make_unique<K4E::Sprite>();
	...
	// 数字描画用
	scoreDrawer_ = std::make_unique<K4E::NumberSpriteDrawer>();
	scoreDrawer_->Initialize("number.png");

	// キル数描画用
	killDrawer_ = std::make_unique<K4E::NumberSpriteDrawer>();
	killDrawer_->Initialize("number.png");

	/*waveDrawer_ = std::make_unique<K4E::NumberSpriteDrawer>();
...
{
	// マウス座標取得
	K4E::Vector2 mousePos = K4E::Input::GetInstance()->GetMousePosition();

	// リスタートボタンの範囲判定
	K4E::Vector2 restartPos = restartText_->GetPosition();
	K4E::Vector2 restartSize = restartText_->GetSize();
	if (K4E::Input::GetInstance()->TriggerMouse(0))
	{
	...
	// タイトルへ戻るボタンの範囲判定
	K4E::Vector2 titlePos = titleText_->GetPosition();
	K4E::Vector2 titleSize = titleText_->GetSize();
	if (K4E::Input::GetInstance()->TriggerMouse(0))
	{
