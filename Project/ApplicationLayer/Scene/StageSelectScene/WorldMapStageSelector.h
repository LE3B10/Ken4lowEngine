...
///				ワールドマップ型ステージセレクター
/// -------------------------------------------------------------
class WorldMapStageSelector : public IStageSelector
{
public: /// ---------- メンバ関数 ---------- ///

	// 仮想デストラクタ
	virtual ~WorldMapStageSelector() = default;
	...
	// 外部からの指示用
	void FocusToIndex(int index, bool tween = true) override; // 指定インデックスへフォーカスs
...
private: /// ---------- メンバ関数 ---------- ///
	...
	void BuildSlots(); // 画面サイズに合わせてスロット座標を組む

	int   GetNextPlayableIndex() const; // 最初の未クリア/アンロック面を返す

	void TriggerLockedShake(int index);  // 対象カードを指定
...
private: /// ---------- メンバ変数 ---------- ///
	...
	std::unique_ptr<Sprite> selFrame_;  // 選択枠
	...
	std::array<Vector2, 9> slots_{};

	// -- - aura(next - to - play) -- -
	std::unique_ptr<Sprite> aura_;
	float auraTimer_ = 0.0f;
	float auraSpeed_ = 3.8f;      // 脈動速度
	float auraMin_ = 1.08f;     // 最小スケール
	float auraMax_ = 1.32f;     // 最大スケール
	int shakeTargetIndex_ = -1;

	int hoverIndex_ = -1;  // いまカーソルが乗っているカード（-1で無し）
};
