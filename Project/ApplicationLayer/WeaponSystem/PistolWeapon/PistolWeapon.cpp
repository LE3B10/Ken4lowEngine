...
{
	K4E::Collider::SetTypeID(static_cast<uint32_t>(CollisionTypeIdDef::kWeapon));
...
///				　			　 衝突時処理
/// -------------------------------------------------------------
void PistolWeapon::OnCollision(K4E::Collider* other)
{
...
///				　			　 中心座標取得
/// -------------------------------------------------------------
K4E::Vector3 PistolWeapon::GetCenterPosition() const
{
	// 行列を最新に更新
	const_cast<K4E::WorldTransformEx&>(transform_).Update();

	// オフセット（ローカル座標系）
	const K4E::Vector3 offet = { 0.0f,0.0f,0.0f };

	// ワールド行列を使って座標変換
	position_ = K4E::Matrix4x4::Transform(offet, transform_.worldMatrix_);
