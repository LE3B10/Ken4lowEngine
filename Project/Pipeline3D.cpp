...
Pipeline3D::Pipeline3D(DirectXCommon* dxCommon) : dxCommon_(dxCommon) {}

void Pipeline3D::Initialize()
{
	// パイプラインの生成
	CreatePSO();
}

void Pipeline3D::Render()
{
...
void Pipeline3D::CreateRootSignature()
{
	// RootSignatureの設定
...
void Pipeline3D::CreatePSO()
{
	...
	// Shaderをコンパイル
	ComPtr <IDxcBlob> vertexShaderBlob = ShaderManager::CompileShader(L"Resources/Shaders/Object3D.VS.hlsl", L"vs_6_0", dxCommon_->GetIDxcUtils(), dxCommon_->GetIDxcCompiler(), dxCommon_->GetIncludeHandler());
	assert(vertexShaderBlob != nullptr);

	// Pixelをコンパイル
	ComPtr <IDxcBlob> pixelShaderBlob = ShaderManager::CompileShader(L"Resources/Shaders/Object3D.PS.hlsl", L"ps_6_0", dxCommon_->GetIDxcUtils(), dxCommon_->GetIDxcCompiler(), dxCommon_->GetIncludeHandler());
