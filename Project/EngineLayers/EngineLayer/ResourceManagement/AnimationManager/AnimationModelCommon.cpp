...
///					シングルトンインスタンス
/// -------------------------------------------------------------
AnimationModelCommon* AnimationModelCommon::GetInstance()
{
	static AnimationModelCommon instance;
	return &instance;
}
...
///							初期化処理
/// -------------------------------------------------------------
void AnimationModelCommon::Initialize(DirectXCommon* dxCommon)
{
...
///				　ルートシグネチャを生成する処理
/// -------------------------------------------------------------
void AnimationModelCommon::CreateRootSignature()
{
	// RootSignatureの設定
...
///					パイプラインを生成する処理
/// -------------------------------------------------------------
void AnimationModelCommon::CreatePSO()
{
	HRESULT hr{};
	...
	inputElementDescs[4] = { "INDEX",    0, DXGI_FORMAT_R32G32B32A32_SINT,	0, D3D12_APPEND_ALIGNED_ELEMENT, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
	...
	//Shaderをコンパイルする
	Microsoft::WRL::ComPtr <IDxcBlob> vertexShaderBlob = shaderManager_->CompileShader(L"Resources/Shaders/SkinningObject3d.VS.hlsl", L"vs_6_0", dxCommon_->GetIDxcUtils(), dxCommon_->GetIDxcCompiler(), dxCommon_->GetIncludeHandler());
	assert(vertexShaderBlob != nullptr);

	//Pixelをコンパイルする
	Microsoft::WRL::ComPtr <IDxcBlob> pixelShaderBlob = shaderManager_->CompileShader(L"Resources/Shaders/SkinningObject3d.PS.hlsl", L"ps_6_0", dxCommon_->GetIDxcUtils(), dxCommon_->GetIDxcCompiler(), dxCommon_->GetIncludeHandler());
